  ...
  return (
    ...
    <main className="mx-auto flex min-h-screen w-full items-center justify-center bg-slate-900 text-white">
      ...
      <form onSubmit={handleSubmit} className="flex w-[30rem] flex-col space-y-10 blue-glassmorphism p-12">
        ...
        <button type="submit" className="transform rounded-full bg-yellow-700 py-2 px-7 mx-auto font-bold duration-300 hover:bg-yellow-400">
          {isRegister ? 'Register' : isForgotPassword ? 'RESET PASSWORD' : 'Log In'}
        </button>

        {!isRegister && !isForgotPassword && (
          <div onClick={handleForgotPassword} className="transform text-center font-semibold text-gray-500 duration-300 hover:text-yellow-400">
            FORGOT PASSWORD?
          </div>
        )}
        {isForgotPassword && (
